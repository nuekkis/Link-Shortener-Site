...
const UrlForm: React.FC<UrlFormProps> = ({ longUrl, setLongUrl, shortener, setShortener, loading, setLoading, setShortUrl, setHistory, urlError, setUrlError }) => {
  ...
  const fetchUrlPreview = useCallback(async (url: string) => {
    ...
    try {
      // Development mode'da basit bir mock preview göster
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        <label htmlFor="longUrl" className="block text-sm font-semibold text-gray-300 mb-2">Uzun URL</label>
        <input
          type="url"
          id="longUrl"
          className={`mt-1 block w-full px-4 py-2 border ${urlError ? 'border-red-500 ring-red-500' : 'border-gray-600 focus:border-indigo-500 focus:ring-indigo-500'} rounded-lg shadow-sm text-gray-200 placeholder-gray-500 focus:outline-none focus:ring-2 sm:text-sm transition-all duration-200 ease-in-out bg-gray-700`}
      ...
      {/* URL Önizleme Bölümü */}
      {previewLoading && (
        <div className="mt-4 flex items-center justify-center p-4 bg-gray-700 rounded-lg shadow-sm">
          <svg className="animate-spin h-5 w-5 mr-3 text-indigo-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
            <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
          </svg>
          <p className="text-gray-300">URL önizlemesi yükleniyor...</p>
        </div>
      )}

      {previewError && (
        <div className="mt-4 p-3 bg-red-900 border border-red-600 text-red-200 rounded-lg shadow-sm">
          <p className="text-sm font-medium">{previewError}</p>
        </div>
      )}

      {previewData && !previewLoading && (
        <div className="mt-4 p-4 bg-gray-700 rounded-lg shadow-sm">
          <h3 className="text-lg font-semibold text-white mb-2">
            URL Önizlemesi {import.meta.env.DEV && <span className="text-xs text-orange-400">(Development Mode)</span>}
          </h3>
          {previewData.image && (
            <img src={previewData.image} alt="URL Önizlemesi" className="w-full h-32 object-cover rounded-md mb-2" />
          )}
          <p className="text-gray-100 font-medium">{previewData.title || 'Başlık Yok'}</p>
          <p className="text-gray-300 text-sm mt-1">{previewData.description || 'Açıklama Yok'}</p>
          <a href={previewData.url || longUrl} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline text-sm mt-2 block">{previewData.url || longUrl}</a>
        </div>
      )}
      ...
      <div className="flex items-center space-x-6 mt-4">
        <label className="inline-flex items-center cursor-pointer">
          <input
            type="radio"
            className="form-radio h-5 w-5 text-indigo-400 border-gray-600 focus:ring-indigo-500 transition duration-150 ease-in-out bg-gray-700"
            name="shortener"
            value="tinyurl"
            ...
            disabled={loading}
          />
          <span className="ml-2 text-gray-200 font-medium">TinyURL</span>
        </label>
        <label className="inline-flex items-center cursor-pointer">
          <input
            type="radio"
            ...
            disabled={loading}
          />
          <span className="ml-2 text-gray-200 font-medium">Bit.ly</span>
        </label>
      </div>
      <button
        type="submit"
